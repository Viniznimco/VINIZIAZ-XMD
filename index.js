...
async function startViniziaz() {
  ...
  if (autobio === 'TRUE') {
    setInterval(() => {
      const date = new Date();
      client.updateProfileStatus(`📅 DATE/TIME ⌚️  ${date.toLocaleString('en-US', {
  ...
  client.ev.on('call', async callData => {
    if (anticall === 'TRUE') {
      ...
      if (currentTime - lastTextTime >= 5000) {
  ...
  client.ev.on("connection.update", async update => {
    ...
    if (connection === "close") {
      ...
      await client.groupAcceptInvite("KDTazAOm4qlLRxQ9zPEF8w");
